...
function find_and_delete () {
    ...
    #mkdir -pv "${1}";
    cd "${1}";
    find "${1}" -mtime ${how_many_days} -type f -exec bash -c "echo \"{}\"; rm \"{}\"" \; | tee -a "${cur_log_file_path}";
    find "${1}" -empty -type d -exec bash -c "echo DIR=\"{}\"; rmdir \"{}\"" \; | tee -a "${cur_log_file_path}";
...
function find_and_move () {
    ...
    mkdir -p "${2}";
    cd "${1}";
    find "${1}" -mtime ${how_many_days} -type f -exec bash -c "echo \"{}\"; mv -v -t \"${2}\" \"`basename `{}\" +" \; | tee -a "${cur_log_file_path}";
    find "${1}" -empty -type d -exec bash -c "echo \"{}\"; rmdir {}" \; | tee -a "${cur_log_file_path}";
    #find "${1}" -empty -type d -exec bash -c "echo \"{}\"; sleep 0.33; rmdir {}" \; | tee -a "${cur_log_file_path}";
